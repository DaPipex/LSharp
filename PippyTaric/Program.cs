    ...
    class Program
    {
        ...
        */
        ...
        private static void Combo()
        {
            ...
            var allyList = ObjectManager.Get<Obj_AI_Hero>().ToList().FindAll(ally => ally.IsAlly);
            var enemyList = ObjectManager.Get<Obj_AI_Hero>().ToList().FindAll(enemy => enemy.IsEnemy);
        ...
        private static void TaricBuffRemove(Obj_AI_Base sender, Obj_AI_BaseBuffRemoveEventArgs args)
        {
            if (sender.IsMe && args.Buff.Name == buffName)
            {
                hasPassive = false;
